...
export default class ChatWhisperUsers extends ChatMenu {
  ...
  filter() {
    [...this.chat.whispers.values()].forEach((whisper) => {
      if (whisper.nick.toLowerCase().indexOf(this.searchterm.toLowerCase()) >= 0) {
        whisper.found = true;
      } else {
        whisper.found = false;
      }
  ...
  redraw() {
    ...
    if (this.chat.whispers.size === 0) {
      ...
      [...this.chat.whispers.values()].sort((a, b) => new Date(b.time).getTime() - new Date(a.time).getTime()).forEach((whisper) => this.addConversation(whisper));
    }
  ...
  addConversation(whisper) {
    const date = new Date(whisper.time);
    const found = whisper.found ? ' found' : '';
    (whisper.unread > 0 ? this.unreadEl : this.readEl).append(`
    <div class="user-entry ${whisper.online ? 'online' : 'offline'}${this.searchterm.length > 0 ? found : ''}" title="${whisper.online ? 'online' : 'offline'}" data-username="${whisper.nick.toLowerCase()}">
      <span class="user">${whisper.nick}</span>
      <div class="right">
        ${whisper.unread > 0 ? `<span class="unread">${whisper.unread} new</span>` : ''}
        <span class="time">${date.getDate()}/${date.getMonth()+1}/${date.getFullYear()-2000}</span>
      </div>
    </div>`);
  }
}
